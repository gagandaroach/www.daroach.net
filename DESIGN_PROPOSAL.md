...
> 4. ~~**`_old/` not yet archived**~~ ✅ **DONE 2026-06-08** — `_old/` is now an asset-stripped **collation of all pre-`dev-2026` branches** (`main`, `dev`, `cursor`, `nuxt3`, `main-bk1`; `post-cassie` as a patch; `main-bkup` ≡ `main-bk1`), each with a `THEME.md`. See `_old/README.md` + `_old/ASSETS-INVENTORY.md`. Remote branches left intact (not pruned). §9.2 resolved.
...
**Phase 1 — Shell & brand. ✅ DONE (2026-06-08, commit `1a6d790` on `dev-2026`).** `layouts/default.vue` (NavBar + slot + Footer) + `layouts/fullscreen.vue` (bare, for the hero). `components/layout/{NavBar,Footer,ThemeToggle}.vue`; Footer carries the consent-reset control. `useTheme()` wraps `useColorMode()`; `useConsent()` records the (cookieless, exempt) transparency-notice dismissal via `@vueuse` `useStorage` (no Pinia — kept for the dashboard). Theme toggle works light↔dark with no flash (icon wrapped in `<ClientOnly>`); only the dark palette is polished per the dark-only-v1 decision. `.dnet-card`/`.dnet-button` ported as the first `tailwind-variants` primitives — `components/ui/Button.vue` (`<UiButton>`, solid/outline/ghost × sm/md/lg, renders `<NuxtLink>` when `to` is set) and `components/ui/Card.vue` (`<UiCard>`, the bg-surface/blur card with the red-on-hover border + -2px lift) — establishing the §5.4a pattern. Real fullscreen hero at `/` (`app/pages/index.vue`, `fullscreen` layout): wordmark, identity line, CTA buttons into Timeline/Blog/Dashboard/About, a drifting masked grid + red glow backdrop (successor to `shiftTiles`, honors `prefers-reduced-motion`). `npm run build` green.

> **New decisions/deferrals (Phase 1):**
> - **`tailwind-merge` added as an explicit dep.** `tailwind-variants@3` declares it as an *optional* peer, so the Vite build fails with `"twMerge" is not exported` until it's installed. Added `tailwind-merge@^3` (v3 supports Tailwind v4). The §0/§8 note that tv "pulls in tailwind-merge" is true at runtime but it must be installed explicitly.
> - **Baseline build env note (this x64-linux machine):** a stale/partial `node_modules` left `better-sqlite3`'s native binding unresolved (`Could not locate the bindings file`); a clean `rm -rf node_modules && npm install` rebuilt it correctly. `sharp` resolves for `linux-x64` here (the Phase 0 darwin-arm64 mismatch is moot on this box; still a real Phase 6 concern for the container arch).
> - Node is **v20.20.2** on this machine (some deps warn `EBADENGINE` wanting Node 22). Build is green regardless; the Dockerfile already targets `node:22-alpine`.

**Phase 2 — Content. ✅ DONE (2026-06-08, commit `d09b658` on `dev-2026`).** `/blog` (`pages/blog/index.vue`, lists via `queryCollection('blog').order('date','DESC')`, drafts filtered in JS) + `/blog/[...slug].vue` (`ContentRenderer` + Shiki `github-dark`, configured in `nuxt.config` `content.build.markdown.highlight`). `components/blog/PostCard.vue` (`<BlogPostCard>`, presentational, on `<UiCard interactive>`). Two sample posts. Prose styled via a scoped `:deep(.prose-dnet)` block (no `@tailwindcss/typography` — stays within the locked stack). `/about` with a `#privacy` section documenting the cookieless-analytics model (links from the notice). Transparency notice `components/consent/Notice.vue` (`<ConsentNotice>`) mounted site-wide in `app.vue` under `<ClientOnly>`; dismiss = `useConsent().acknowledge()`, footer reset brings it back. `routeRules`: `/about` prerender, `/blog` + `/blog/**` ISR. `npm run build` green (6 routes prerendered).

> **New decisions/deferrals (Phase 2):**
> - **`@vueuse/nuxt` auto-imports `useLocalStorage`/`useSessionStorage` but NOT the bare `useStorage`.** `useConsent()` initially used `useStorage` → SSR `ReferenceError: useStorage is not defined` (surfaced only now: Phase 1 prerendered just `/`, whose chrome-less `fullscreen` layout never SSR'd the footer that calls `useConsent`). Switched to `useLocalStorage`. Watch for this with other VueUse composables.
> - **Tailwind v4 `@apply` in scoped Vue `<style>` requires `@reference`.** The blog prose block fails (`Cannot apply unknown utility class`) until it starts with `@reference "../../assets/css/main.css"` (scoped styles compile separately from the main sheet). Documented inline.
> - **Shiki theme = `github-dark`** with an explicit `langs` allow-list (ts/js/vue/bash/json/yaml/sql/python/html/css) to keep the highlighter bundle bounded.
...
**Phase 4 — Analytics backend. ✅ DONE (2026-06-08, commit `2c37143` on `dev-2026`).** `server/utils/db.ts` — better-sqlite3 singleton, WAL, `PRAGMA user_version` migration runner, the `hits` table (ts/path/referrer/country/region/ua/visitor_hash) + indexes, `insertHit()`, and throttled (~hourly) `pruneOldHits()` retention. `server/utils/geo.ts` — `resolveClientIp` (CF-Connecting-IP → XFF[0] → getRequestIP), `resolveCountry` (CF-IPCountry, drops XX/T1), `hashVisitor` (HMAC-SHA256 keyed by `salt:utcDay`, daily rotation), `utcDayKey`. `server/api/hit.post.ts` — derives country + visitor hash, **discards the raw IP**, always returns 204, never throws (analytics can't break the page). `useAnalytics()` (`track` via `navigator.sendBeacon`, fetch+keepalive fallback) wired by `plugins/analytics.client.ts` (one hit on mount + one per SPA nav, previous path as referrer; never gated on consent). DB path / salt / retention via server-only runtimeConfig (`NUXT_ANALYTICS_DB_PATH` / `NUXT_IP_HASH_SALT` / `NUXT_ANALYTICS_RETENTION_DAYS`). `routeRules['/api/**'].cors`. `npm run build` green.

> **Verified locally:** POSTed hits with simulated `cf-connecting-ip` + `cf-ipcountry` → rows stored country (US/DE) + a 32-hex salted hash, **the raw IP appears nowhere** in the table, empty beacon recorded nothing, `user_version=1`. Real browser loads of `/` and `/about` recorded via the beacon (same-day same-IP → identical hash, correct for unique counting).
> **Deferred to Phase 6 (needs the cluster — can't be done locally):** end-to-end real-IP verification through Cloudflare→cloudflared→Traefik, which requires Traefik `forwardedHeaders.trustedIPs` = CF CIDRs + the CF "visitor location headers" Managed Transform (else `cf-ipcountry`/`cf-connecting-ip` are absent/untrusted). On localhost there's no CF header, so `country` is null and the IP is `::1` — expected.
> **Note:** the hit endpoint falls back to an insecure `dev-insecure-salt` when `ipHashSalt` is unset (dev only). Prod MUST set `NUXT_IP_HASH_SALT` (Phase 6 secret).

**Phase 5 — Dashboard. ✅ DONE (2026-06-08, commit `0ee5047` on `dev-2026`).** `server/api/stats.get.ts` (totals, last-24h, unique-visitors-24h, country count, top-10 paths, 30-day daily series bucketed in SQL via `strftime`) + `server/api/geo.get.ts` (per-ISO-2 counts). `/dashboard` is `ssr:false` (routeRules). Single Pinia store `app/stores/dashboard.ts` (`@pinia/nuxt` + `pinia` added here per the plan) fetches both via `$fetch` on mount. Components: `dashboard/Stat.vue` (`<DashboardStat>` tiles), `dashboard/WorldMap.vue` (`<DashboardWorldMap>` — flat ECharts choropleth, **no globe** per §0.1), `dashboard/Traffic.vue` (`<DashboardTraffic>` — the old "Web Traffic" card, now a real 30-day hits/visitors line). Shared response types in `shared/types/analytics.ts`. ECharts via `vue-echarts` with tree-shaken `use([...])` registrations. World map: `world-atlas` TopoJSON → GeoJSON (`topojson-client`), matched by ISO-3166 **numeric** (`nameProperty:'numeric'`), CF ISO-2 → numeric via `i18n-iso-countries`. Verified clean in both dev and the production node-server build (8 prerendered routes; dashboard is the SPA route). `npm run build` green.

> **New deps (Phase 5):** `pinia`, `@pinia/nuxt`, `echarts`, `vue-echarts`, `world-atlas`, `topojson-client`, `i18n-iso-countries`.
> **Two echarts gotchas fixed (both would silently break the map):**
> 1. **Never put a string in a series-`data` item's `label` field** — `label` is the reserved per-item label *style object*; a string there throws downstream. Custom display strings go under a non-reserved key (`cname`).
> 2. **Don't let echarts init against a 0×0 container.** A zero-sized container makes the geo transform matrix singular → `matrixInvert` returns null → echarts crashes in `resizeGeo` (`copy2` reads null). Fix: render the chart inside a wrapper with an explicit pixel height and gate `<VChart>` behind a `ready` ref set in `onMounted(()=>nextTick())`, plus `autoresize`. Applied to both charts. (Captured the stack with puppeteer-core driving the local Chrome — handy for SPA-route debugging.)
...
2. ~~**`_old/` + stale branches**~~ ✅ **RESOLVED 2026-06-08.** Per Gagan: *collate all branches into `_old/`, strip assets, summarize each theme; re-port assets later.* Done — `_old/` now holds asset-stripped, summarized snapshots of every pre-`dev-2026` branch (`main`/`dev`/`cursor`/`nuxt3`/`main-bk1`, `post-cassie` as a patch, `main-bkup` noted as an identical mirror), each with a `THEME.md`, plus `_old/README.md` (index) and `_old/ASSETS-INVENTORY.md` (the stripped assets + where to re-port from). The remote branches were **left intact** (not pruned) so the stripped assets stay recoverable.
